...
def _dirichlet_mode_core(c):
    """
    Performs the actual Dirichlet mode operation.

    Args:
        c: Parameter of the Dirichlet.

    Returns:
        The mode of the Dirichlet.
    """
...
def mod_dirichlet_mode(c, a, b):
    ...
    if np.all(less_than_one):
        # If all Dirichlet parameters are below zero, try again increasing them a bit
        return mod_dirichlet_mode(c + 0.1, a, b)

    if np.any(less_than_one):
        # If at least one parameter is below zero (but not all) then, approximate
        # a result by recomputing for c > 1, and consider a zero for the entries
        # where c < 0.

        valid_subset_mode = _dirichlet_mode_core(c[~less_than_one])
        dirichlet_mode = np.zeros(c.size)
        dirichlet_mode[~less_than_one] = valid_subset_mode
    else:
        dirichlet_mode = _dirichlet_mode_core(c)

    # Apply the mod-Dirichlet transformation to the Dirichlet mode
...
def mod_dirichlet_parameters(c, delta, eta):
    ...
    if alpha < 1 and beta < 1:
        # If both parameters are below 1, try again increasing them a little bit
