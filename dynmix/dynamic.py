...
from . import common, dirichlet, dlm, independent
...
def estimator(Y, spec, numit=10, mnumit=100, numeps=1e-6, M=200, phi_priors=None):
    ...
    with tqdm(total=numit * n) as progress_bar:
        for _ in range(numit):
            ...
            W = []
            for j in range(k):
                theta[j], V, Wj, _ = dlm.dynamic_weighted_mle(
                    Y,
                    F_list[j],
                    G_list[j],
                    eta[:, :, j],
...
def sampler(Y, model_specification, model_delta=False, num_samples=2000):
    ...
    for _ in trange(num_samples):
        ...
        sigma = [np.diag(1.0 / phi[j]) for j in range(k)]
        ...
        for j in range(k):
            ...
            for t in range(T):
                ...
                included_observations_real_indexes = [
                    indexes for i in included_obsevations_index for indexes in idx_map[i]
                ]
            ...
            a, R, M, C, W = dlm.kfilter_df_dyn(Y_j, F_j, G_list[j], V_j)
            M, C = dlm.ksmoother(G_list[j], a, R, M, C)
            ...
            for t in range(T):
                ...
                obs_error += (
                    ((np.dot(F_j[t], theta[j][t]) - Y_j[t]) ** 2).reshape((n_j[t], m)).sum(axis=0)
                )
